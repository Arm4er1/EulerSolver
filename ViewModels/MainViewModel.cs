    ...
    public class MainViewModel : BaseViewModel
    {
        ...
        public EquationExample SelectedExample
        {
            ...
            set
            {
                if (SetProperty(ref _selectedExample, value) && value != null)
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(value.ExactText))
                        ExactSolutionHint = "из примера";
                    else
                        ExactSolutionHint = "";
        ...
        private void ExecuteSolve()
        {
            try
            {
                ...
                var equation = new DifferentialEquation
                {
                    Name = "y' = " + EquationText,
                ...
                if (UseRungeControl)
                    result = _solver.SolveWithRungeControl(equation, X0, Y0, Xn, StepH, Epsilon);
                else
                ...
                string summary = "Уравнение: " + result.EquationDescription + "\n" +
                                 "Метод: Модифицированный Эйлер (Эйлер-Коши)\n" +
                                 "Отрезок: [" + result.X0.ToString("F4") + "; " + result.Xn.ToString("F4") + "]\n" +
                ...
                if (result.MaxAbsoluteError.HasValue)
                    summary += "\nМакс. абс. погрешность: " + result.MaxAbsoluteError.Value.ToString("E4");

                ResultSummary = summary;
                StatusText = "✔ Решение получено за " + result.ElapsedMilliseconds.ToString("F3") +
